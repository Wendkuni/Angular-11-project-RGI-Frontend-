...
var AuthAdminService = /** @class */ (function () {
    function AuthAdminService(token, route) {
        this.token = token;
        this.route = route;
    }
    AuthAdminService.prototype.canActivate = function (route, state) {
        if (this.token.getUser().roles[0].name === 'ROLE_ADMIN' || this.token.getUser().roles[0].name === 'ROLE_MATDS') {
            return true;
        }
        else {
